...
This command can be run multiple times in the same working directory as long as you use the same value for the `-l`/`--sequenceLength` argument. Duplicates will not be created. This is useful since the number of possible oligopeptides is huge for anything but very small lengths, so you may want to generate the SMILES in batches
